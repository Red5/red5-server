...
import org.red5.server.net.httpflv.FLVStreamWriter;
...
/**
 * Represents a WebSocket-FLV connection to a client.
 * This class manages the lifecycle of a WebSocket-FLV streaming connection,
 * subscribing to a broadcast stream and delivering FLV data over WebSocket binary frames.
 *
 * Implements IStreamListener to receive packets from the broadcast stream
 * and converts them to FLV tag format for WebSocket delivery.
 *
 * Compatible with flv.js WebSocket mode.
 *
 * @author Paul Gregoire (mondain@gmail.com)
 */
public class WebSocketFLVConnection implements IStreamListener {

    private static final Logger log = LoggerFactory.getLogger(WebSocketFLVConnection.class);
    ...
    /**
     * Creates a new WebSocket-FLV connection.
     *
     * @param wsConnection The underlying WebSocket connection
     * @param scope The Red5 scope this connection belongs to
     * @param streamName The name of the stream to subscribe to
     */
    public WebSocketFLVConnection(WebSocketConnection wsConnection, IScope scope, String streamName) {
        this.wsConnection = wsConnection;
        this.scope = scope;
        this.streamName = streamName;
        this.createdTime = System.currentTimeMillis();
        this.lastActivity = createdTime;
        log.debug("Created WebSocket-FLV connection: {} for stream: {} in scope: {}", wsConnection.getSessionId(), streamName, scope.getName());
    }
    ...
    public boolean initialize() {
        if (!connected.get() || !wsConnection.isConnected()) {
            return false;
        }
        try {
            // Send FLV header
            if (headerSent.compareAndSet(false, true)) {
                ...
                wsConnection.send(header);
                bytesSent.addAndGet(header.length);
                lastActivity = System.currentTimeMillis();
                log.debug("Sent FLV header for WebSocket connection: {}", wsConnection.getSessionId());
            }
            return true;
        } catch (IOException e) {
            log.debug("Failed to initialize WebSocket-FLV connection {}: {}", wsConnection.getSessionId(), e.getMessage());
            close();
            return false;
        }
    }
    ...
    public boolean sendInitialData(StreamConfiguration config) {
        ...
        try {
            ...
            if (metadata != null && metadataSent.compareAndSet(false, true)) {
                sendData(metadata);
                log.debug("Sent metadata for WebSocket connection: {}", wsConnection.getSessionId());
            }
            ...
            if (videoConfig != null && videoConfigSent.compareAndSet(false, true)) {
                ...
                log.debug("Sent video decoder config for WebSocket connection: {}", wsConnection.getSessionId());
            }
            ...
            if (audioConfig != null && audioConfigSent.compareAndSet(false, true)) {
                ...
                log.debug("Sent audio decoder config for WebSocket connection: {}", wsConnection.getSessionId());
            }
            ...
            if (gopFrames != null && gopFrames.length > 0 && gopCacheSent.compareAndSet(false, true)) {
                ...
                log.debug("Sent {} GOP cache frames for WebSocket connection: {}", gopFrames.length, wsConnection.getSessionId());
            }
            return true;
        } catch (IOException e) {
            log.debug("Failed to send initial data for WebSocket connection {}: {}", wsConnection.getSessionId(), e.getMessage());
            close();
            return false;
        }
    }
                ...
                if (videoCodec != null) {
                    ...
                    if (decoderConfig != null && decoderConfig.hasRemaining()) {
                        ...
                        if (flvTag != null && videoConfigSent.compareAndSet(false, true)) {
                            sendData(flvTag);
                            log.debug("Sent video decoder config from stream for WebSocket connection: {}", wsConnection.getSessionId());
                        }
                    }
                }
            }
                ...
                if (audioCodec != null) {
                    ...
                    if (decoderConfig != null && decoderConfig.hasRemaining()) {
                        ...
                        if (flvTag != null && audioConfigSent.compareAndSet(false, true)) {
                            sendData(flvTag);
                            log.debug("Sent audio decoder config from stream for WebSocket connection: {}", wsConnection.getSessionId());
                        }
                    }
                }
            }
                ...
                if (metaData != null) {
                    ...
                    if (data != null && data.hasRemaining()) {
                        ...
                        if (flvTag != null && metadataSent.compareAndSet(false, true)) {
                            sendData(flvTag);
                            log.debug("Sent metadata from stream for WebSocket connection: {}", wsConnection.getSessionId());
                        }
                    }
                }
            }
        } catch (IOException e) {
            log.debug("Failed to send codec config from stream for WebSocket connection {}: {}", wsConnection.getSessionId(), e.getMessage());
        }
    }
    ...
    public void subscribe(IBroadcastStream stream) {
        if (stream == null) {
            log.warn("Cannot subscribe to null stream for WebSocket connection: {}", wsConnection.getSessionId());
            return;
        }
        ...
        log.info("WebSocket-FLV connection {} subscribed to stream: {}", wsConnection.getSessionId(), stream.getPublishedName());
    }
    ...
    public void unsubscribe() {
        if (broadcastStream != null) {
            ...
            log.debug("WebSocket-FLV connection {} unsubscribed from stream", wsConnection.getSessionId());
    ...
    public void packetReceived(IBroadcastStream stream, IStreamPacket packet) {
        ...
        try {
            ...
            log.debug("Error processing packet for WebSocket connection {}: {}", wsConnection.getSessionId(), e.getMessage());
            close();
        }
    }
    ...
    private boolean handleVideoPacket(IStreamPacket packet) {
        ...
        if (codecId == 7 || codecId == 12) { // 7 = AVC, 12 = HEVC
            ...
            if (secondByte == 0) {
                ...
                log.debug("Video codec configuration received for WebSocket connection: {} (codec: {})", wsConnection.getSessionId(), codecId == 7 ? "AVC" : "HEVC");
                ...
                log.trace("Skipping video frame before codec config for WebSocket connection: {}", wsConnection.getSessionId());
                return false;
            }
        }
        ...
        if (!isConfigPacket) {
            if (isKeyframe) {
                ...
                log.trace("Skipping inter frame while waiting for keyframe for WebSocket connection: {}", wsConnection.getSessionId());
                return false;
    ...
    private boolean handleAudioPacket(IStreamPacket packet) {
        ...
        if (codecId == 10) { // AAC
            ...
            if (secondByte == 0) {
                ...
                log.debug("Audio codec configuration received for WebSocket connection: {}", wsConnection.getSessionId());
                // Update stream config if available
                if (streamConfig != null) {
                ...
                log.trace("Skipping audio frame before codec config for WebSocket connection: {}", wsConnection.getSessionId());
                return false;
            }
        }
        return true;
    ...
    /**
     * Sends raw data via WebSocket binary frame.
     *
    ...
    /**
     * Closes the WebSocket-FLV connection.
     */
    public void close() {
        if (connected.compareAndSet(true, false)) {
            log.debug("Closing WebSocket-FLV connection: {}", wsConnection.getSessionId());
            // Unsubscribe from stream
            unsubscribe();
            log.info("WebSocket-FLV connection closed: {} (packets: {}, bytes: {})", wsConnection.getSessionId(), packetsSent.get(), bytesSent.get());
        }
    }
    ...
    public boolean isConnected() {
        return connected.get() && wsConnection.isConnected();
    }
    ...
    /**
     * Gets the connection ID (WebSocket session ID).
     *
     * @return connection ID
     */
    public String getConnectionId() {
        return wsConnection.getSessionId();
    }
    ...
    public String toString() {
        return "WebSocketFLVConnection{" + "connectionId='" + wsConnection.getSessionId() + '\'' + ", streamName='" + streamName + '\'' + ", scope=" + (scope != null ? scope.getName() : "null") + ", connected=" + isConnected() + ", packetsSent=" + packetsSent.get() + ", bytesSent=" + bytesSent.get() + ", lastActivity=" + lastActivity + '}';
    }
