...
public class OBUParser {
    ...
    */
    public static OBUInfo getNextObu(byte[] buf, int bufSize) throws OBUParseException {
        int pos = 0;
        boolean obuExtensionFlag;
        boolean obuHasSizeField;
        OBUInfo info = new OBUInfo();
        if (bufSize < 1) {
            throw new OBUParseException("Buffer is too small to contain an OBU.");
        }
        info.obuType = OBPOBUType.values()[(buf[pos] & 0x78) >> 3];
        obuExtensionFlag = (buf[pos] & 0x04) != 0;
        obuHasSizeField = (buf[pos] & 0x02) != 0;
        pos++;
        if (!isValidObu(info.obuType)) {
            throw new OBUParseException("OBU header contains invalid OBU type: " + info.obuType.getValue());
        }
        if (obuExtensionFlag) {
            if (bufSize < pos + 1) {
        ...
        if (obuHasSizeField) {
            int bytesAvailable = Math.min(4, (bufSize - pos));
            // int will only be 4 bytes long max
            info.offset = pos + bytesAvailable;
            byte[] szBuf = Arrays.copyOfRange(buf, pos, info.offset);
            info.size = LEB128.encode(szBuf);
        } else {
            info.offset = pos;
            info.size = bufSize - pos;
        }
        if (info.size > bufSize - info.offset) {
    ...
    public static OBPSequenceHeader parseSequenceHeader(byte[] buf, int bufSize) throws OBUParseException {
        ...
        if (seq.colorConfig.colorDescriptionPresentFlag) {
            ...
            seq.colorConfig.colorPrimaries = OBPColorPrimaries.OBP_CP_UNSPECIFIED;
            seq.colorConfig.transferCharacteristics = OBPTransferCharacteristics.OBP_TC_UNSPECIFIED;
            seq.colorConfig.matrixCoefficients = OBPMatrixCoefficients.OBP_MC_UNSPECIFIED;
        }
        if (seq.colorConfig.monoChrome) {
            ...
            seq.colorConfig.chromaSamplePosition = OBPChromaSamplePosition.OBP_CSP_UNKNOWN;
            seq.colorConfig.separateUvDeltaQ = false;
        } else if (seq.colorConfig.colorPrimaries == OBPColorPrimaries.OBP_CP_BT_709 && seq.colorConfig.transferCharacteristics == OBPTransferCharacteristics.OBP_TC_SRGB && seq.colorConfig.matrixCoefficients == OBPMatrixCoefficients.OBP_MC_IDENTITY) {
    ...
    */
    private static int parseFrameHeader(byte[] buf, int bufSize, OBPSequenceHeader seq, OBPState state, int temporalId, int spatialId, OBPFrameHeader fh, AtomicBoolean seenFrameHeader) throws OBUParseException {
        ...
        if (seq.reducedStillPictureHeader) {
            ...
            fh.frameType = OBPFrameType.OBP_KEY_FRAME;
            frameIsIntra = true;
            fh.showFrame = true;
            ...
            if (fh.showExistingFrame) {
                ...
                if (fh.frameType == OBPFrameType.OBP_KEY_FRAME) {
            ...
            frameIsIntra = (fh.frameType == OBPFrameType.OBP_INTRA_ONLY_FRAME || fh.frameType == OBPFrameType.OBP_KEY_FRAME);
            ...
            if (fh.showFrame) {
                fh.showableFrame = (fh.frameType != OBPFrameType.OBP_KEY_FRAME);
            } else {
                fh.showableFrame = br.readBits(1) != 0;
            }
            if (fh.frameType == OBPFrameType.OBP_SWITCH_FRAME || (fh.frameType == OBPFrameType.OBP_KEY_FRAME && fh.showFrame)) {
                fh.errorResilientMode = true;
            } else {
                fh.errorResilientMode = br.readBits(1) != 0;
            }
        }
        if (fh.frameType == OBPFrameType.OBP_KEY_FRAME && fh.showFrame) {
            for (int i = 0; i < 8; i++) {
                state.refValid[i] = 0;
        ...
        if (fh.frameType == OBPFrameType.OBP_SWITCH_FRAME) {
        ...
        if (fh.showExistingFrame && fh.frameType == OBPFrameType.OBP_KEY_FRAME) {
    ...
    */
    ...
    private static void parseFilmGrainParams(BitReader br, OBPFrameHeader fh, OBPSequenceHeader seq, OBPState state) throws OBUParseException {
        ...
        if (fh.frameType == OBPFrameType.OBP_INTER_FRAME) {
    ...
    private static boolean isValidObu(OBPOBUType type) {
        switch (type) {
    ...
    public static void main(String[] args) {
        byte[] buffer = new byte[12]; // your buffer containing OBU data
        try {
            OBUInfo obuInfo = OBUParser.getNextObu(buffer, buffer.length);
            System.out.println("OBU Type: " + obuInfo.obuType);
            System.out.println("Offset: " + obuInfo.offset);
            System.out.println("Size: " + obuInfo.size);
            System.out.println("Temporal ID: " + obuInfo.temporalId);
            System.out.println("Spatial ID: " + obuInfo.spatialId);
        } catch (OBUParseException e) {
            System.err.println("Error parsing OBU: " + e.getMessage());
        }
    }
